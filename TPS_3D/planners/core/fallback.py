...
def local_fallback(grid, t_goal_best, t_goals, t_goals_parent, closed_set, start, goal, parent, local_fallback_method, open_set, cost_so_far, graph, build_graph):
    """
    ...
    parent : dict
        Path reconstruction structure.
    local_fallback_method : callable
        Function to compute local fallback paths.
    open_set : list
    ...
    -------
    int
        Number of nodes expanded in fallback or global planner.
    """
    if t_goals:
        ...
        if t_best is not None:
            ...
            return create_fallback_path(grid, t_best, t_goals_parent, local_fallback_method, parent, cost_so_far, open_set, goal, start, graph, build_graph)
    
    # No valid local option → fallback to global planner
    return global_fallback(grid, start, goal, t_goal_best, t_goals_parent, closed_set, parent, local_fallback_method)
...
def Astar(grid, start, goal):
    """
    A* path planning in a 3D occupancy grid using 26-connectivity.

    Parameters
    ----------
    grid : ndarray
        3D occupancy grid (0 = free, non-zero = obstacle).
    start : tuple
        Start voxel (x, y, z).
    goal : tuple
        Goal voxel (x, y, z).
    ...
    -------
    path : ndarray
        Planned path from start to goal as a sequence of voxels.
    """
    x_width, y_width, z_width = grid.shape

    # Motions for 26-connectivity 
    motion = []
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
            for dz in [-1, 0, 1]:
    ...
    while open_set:
        ...
        for dx, dy, dz, move_cost in motion:

            # Neighbor:
            nx, ny, nz = cx + dx, cy + dy, cz + dz
            neighbor = (nx, ny, nz)

            if 0 <= nx < x_width and 0 <= ny < y_width and 0 <= nz < z_width:
                ...
                # Check diagonal corners to prevent corner-cutting through obstacle
                steps = abs(dx) + abs(dy) + abs(dz)
                ...
                if steps == 2:
                    ...
                    if dx != 0 and dy != 0:
                        if grid[cx+dx, cy, cz] == 0 or grid[cx, cy+dy, cz] == 0:
                            allowed = True

                    if dx != 0 and dz != 0:
                        if grid[cx+dx, cy, cz] == 0 or grid[cx, cy, cz+dz] == 0:
                            allowed = True

                    if dy != 0 and dz != 0:
                        if grid[cx, cy+dy, cz] == 0 or grid[cx, cy, cz+dz] == 0:
                            allowed = True
                ...
                if steps == 3:
                    ...
                    if grid[cx+dx, cy, cz] == 0:
                        if grid[cx+dx, cy+dy, cz] == 0 or grid[cx+dx, cy, cz+dz] == 0:
                            allowed = True

                    if grid[cx, cy+dy, cz] == 0:
                        if grid[cx+dx, cy+dy, cz] == 0 or grid[cx, cy+dy, cz+dz] == 0:
                            allowed = True

                    if grid[cx, cy, cz+dz] == 0:
                        if grid[cx+dx, cy, cz+dz] == 0 or grid[cx, cy+dy, cz+dz] == 0:
                            allowed = True
                ...
                if (nx, ny, nz) not in cost_so_far or g < cost_so_far[(nx, ny, nz)]:
                    cost_so_far[(nx, ny, nz)] = g
                    f = g + utils.dist((nx, ny, nz), goal)
                    heapq.heappush(open_set, (f, (nx, ny, nz)))
