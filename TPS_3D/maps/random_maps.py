...
import numpy as np
from TPS_3D.maps.core import random_map_utils as utils

def random_map(size, dense, distribution, relative_size, small, large, connectivity_guarantee=False, clustered=False, max_iter=100):
    """
    ...
    Parameters:
        size (int):
            Final map size as a square occupancy grid of shape (size x size x size).
        dense (float):
    ...
    for iter in range(max_iter):
        ...
        while dense > dense_temp:
            ...
            if connectivity_guarantee:
                ...
                if len(fill_idx) > 0:
                    map_obs_cells[fill_idx[:,0], fill_idx[:,1], fill_idx[:,2]] = l
                    dense_temp = utils.calculate_dense(map_obs_cells[1:-1, 1:-1, 1:-1])
                    if dense_temp >= dense:
                        break
            else:
        ...
        map_obs_cells = map_obs_cells[1:-1, 1:-1, 1:-1]
        ...
        if connectivity_guarantee:
            # Search for a path:
            start = (0,0,0)
            goal = ((size//obs_smallest_size)-1,(size//obs_smallest_size)-1,(size//obs_smallest_size)-1)
